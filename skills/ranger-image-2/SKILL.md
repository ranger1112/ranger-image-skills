...
- If the provider returns URL responses, the script downloads them with a byte limit; use `--response-format url` only when the upstream supports it.
...
5. For providers that support URL output, pass `--response-format url`; otherwise omit it or use `--response-format b64_json`.
